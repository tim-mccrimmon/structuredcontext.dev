...
With 900+ payers operating different systems, different criteria formats, and different exchange mechanisms, the only way AI PA agents can function reliably across the ecosystem is if everyone is working from the same interoperability standard. That's what Da Vinci is. Not a vendor framework — a set of HL7 implementation guides that define how PA should move between provider and payer systems, in a format that works the same way regardless of which payer, which EHR, which platform. The patient benefits because the process doesn't break at every system boundary. When we describe a Da Vinci-aligned agentic architecture, we mean a system built on those IGs — designed for the interoperability reality of 900+ payers, not optimized for one.

The four-IG stack is the current reference standard. It's worth being specific about what each piece does and doesn't do.

**CRD (Coverage Requirements Discovery)** fires a CDS Hooks call at order entry. The EHR asks: is PA required for this service, for this payer, for this plan? If yes: what documentation is needed? CRD returns the answer in real time, before the clinician leaves the encounter. This is meaningful progress over portal lookups. It doesn't touch criteria evaluation — it only determines whether the workflow applies.

**DTR (Documentation Templates and Rules)** delivers a CQL-authored questionnaire to the EHR and executes it against the patient's FHIR record. Where structured FHIR data exists, DTR auto-populates questionnaire responses. Where it doesn't — which is most of the clinically relevant information — it surfaces gaps for the clinician to fill.

This is where the current architecture gets closest to machine-executable criteria. CQL is the right approach: deterministic, executable against structured data, authorable by payers, version-controlled. The limitation is adoption and coverage. Most payers haven't yet authored their coverage criteria in CQL. And even where CQL questionnaires exist, 80% of clinically relevant information lives in free-text notes — prior treatment failures, diagnostic findings, clinical reasoning — that CQL can't query.

**CDex (Clinical Data Exchange)** handles Task-based requests for additional clinical documentation from the provider EHR when DTR has gaps. If the questionnaire requires evidence that the patient failed six weeks of conservative therapy and that fact lives in a physician's note rather than a FHIR Observation, CDex is how the agent requests it. It gets the document. It doesn't evaluate what's in it.
...
The Da Vinci stack solves the protocol and transport problem. It establishes how PA requests move between EHR and payer in a structured, computable format. CQL/DTR is the right direction for machine-executable criteria, and payer adoption will continue to grow. But the gap between where CQL coverage is today and where it needs to be is not a gap that DTR itself closes — it's a gap that requires governed, authoritative criteria to travel with each agent regardless of whether the payer has authored CQL yet.
...
| Clinical Data | DTR + CDex | Executes CQL, NLP on unstructured notes, CDex requests |
...
The current Da Vinci architecture provides protocol and transport (FHIR, X12), tool connectivity (MCP), deterministic rule execution where CQL coverage exists (DTR), and clinical data exchange (CDex). What it doesn't include is a versioned, authoritative, structurally enforced governance context that travels with each agent before any session begins. Not a prompt. Not a guardrail. A declared artifact, sourced from a registry, that tells each agent what it is, what it's authorized to do, which version of which standards it's operating under, and how to treat the runtime inputs it receives.
